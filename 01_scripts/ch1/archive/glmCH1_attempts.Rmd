...
# GLM #5: Number of bees foraging on vaco 
...
```
Look at area level to see if number of nest-searching queens increases number of blueberry foraging

```{r}
area_summary <- field_data %>%
  group_by(area) %>%
  summarise(
    total_forage_vaco = sum(forage_vaco, na.rm = TRUE),
    total_nest_queens = sum(nest_searching_queens, na.rm = TRUE),
    total_workers = sum(workers, na.rm = TRUE),
    mean_bombus_floral_abun = mean(bombus_floral_abun, na.rm = TRUE),
    n_plots = n()
  )

area_summary$area <- as.factor(area_summary$area)


##NSQ GLM

forageVACO_glm_area <- glmmTMB(total_forage_vaco ~ total_nest_queens +
                            mean_bombus_floral_abun,
                         family = poisson,
                         data = area_summary
                         )

residuals_forageVaco_lm <- simulateResiduals(fittedModel = forageVACO_glm_area, plot = TRUE)
#looks good!

#check summary
summary(forageVACO_glm_area)
###significant effect of NSQ on foraging vaco

##Worker GLM
worker_glm_area_pois <- glmmTMB(total_workers ~ total_nest_queens +
                            mean_bombus_floral_abun,
                         family = poisson,
                         data = area_summary
                         )

residuals_worker_glm_area_pois <- simulateResiduals(fittedModel = worker_glm_area_pois, plot = TRUE)
#significant deviations, try negative binomial

worker_glm_area_nbin <- glmmTMB(total_workers ~ total_nest_queens +
                            mean_bombus_floral_abun,
                         family = nbinom2,
                         data = area_summary
                         )

residuals_worker_glm_area_nbin <- simulateResiduals(fittedModel = worker_glm_area_nbin, plot = TRUE)
#looks good!

#check summary
summary(worker_glm_area_nbin)
###no effect of either NSQ or floral abundance on workers
