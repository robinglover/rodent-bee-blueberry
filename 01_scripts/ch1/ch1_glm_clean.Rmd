...
![](images/pathDiagramOutlineOct272025.jpg)
...
1)  Try different random effect structures in the poisson. Compare models using likelihood ratio tests to determine best random effect structure. I did not include plot as a random effect structure since there was only one observation per plot for rodent burrow data.
...
6)  Stick with poisson but try using categorical cover_type as predictor instead of % grass/forb cover and check diagnostics.

-   none of the tests were significant! –\> good to keep this one

```{r}
burrows_pois_cover <- glmmTMB(num_burrows ~ cover_type + 
                             (1|site_id),
                           family = poisson, 
                           data = field_data)
...
![](images/pathDiagramFinalOct272025.jpg)
