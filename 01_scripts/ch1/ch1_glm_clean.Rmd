...
grass_seq <- seq(
  from = min(field_data$grass_forb_perc_scaled, na.rm = TRUE),
  to = max(field_data$grass_forb_perc_scaled, na.rm = TRUE),
  length.out = 50
)
...
Predicted num_burrows vs Grass/Forb %
...
pred_rodent_veg <- predict(burrows_pois_cover,
...
ggplot(pred_rodent_veg_df, aes(x = grass_forb_perc_scaled, y = fit)) +
  geom_line(size = 1) +
...
Predicted number of burrows vs julian date

```{r}
# Define a sequence of values for julian.date_scaled across its observed range
date_seq <- seq(
  from = min(field_data$julian.date_scaled, na.rm = TRUE),
  to = max(field_data$julian.date_scaled, na.rm = TRUE),
  length.out = 50
...
# Get predicted probabilities and standard errors
pred_rodent_date <- predict(burrows_pois_cover,
                            newdata = rodent_data_date,
                            se.fit = TRUE,
                            re.form = NA)

# Combine predictions with data frame
pred_rodent_date_df <- cbind(rodent_data_date,
                 fit = pred_rodent_date$fit,
                 se = pred_rodent_date$se.fit)

ggplot(pred_rodent_date_df, aes(x = julian.date_scaled, y = fit)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = fit - 1.96*se, ymax = fit + 1.96*se), alpha = 0.2) +
  labs(title = "Model Predictions (Log-scale)",
       x = "Julian Date (scaled)",
...
# Get predicted probabilities and standard errors
pred_rodent_cover <- predict(burrows_pois_cover,
