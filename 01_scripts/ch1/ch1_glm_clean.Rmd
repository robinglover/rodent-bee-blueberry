...
surveys$prop_inactive <- surveys$num_inactive_burrows / surveys$num_burrows
#scale
...
##r = -0.149. This means that the vacancy probability and rodent burrow abundance are largely independent. This means we can include both in our models.
...
ggplot(pred_floral_veg_df, aes(x = cover_type, y = response)) +
  geom_point(size = 8) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.2, linewidth = 1.5) +
  labs(
    x = "Cover type",
    y = "Predicted floral\nabundance (log-scale)"
  ) +
...
-   Indicates significant combined adjusted quantile test.

```{r}
residuals_nsq_noRE <- simulateResiduals(fittedModel =NSQ_noRE, plot = TRUE)
```

3.  Try ZI negative binomial

    ```{r}
    NSQ_noRE_nbin <- glmmTMB(nest_searching_queens ~ cover_type + 
                        bombus_floral_abun_scaled + 
                        num_burrows_scaled + 
                        prop_inactive_scaled +
                        bee_season_type, 
...
4.  Check diagnostic plot
    ...
    ```{r}
    residuals_nsq_noRE_nbin <- simulateResiduals(fittedModel =NSQ_noRE_nbin, plot = TRUE)
    ```

5.  Look at results

```{r}
summary(NSQ_noRE_nbin)
```
...
-   Significant combined adjusted quantile test.
...
-   Residual plot shows better fit and no significant deviations.
