...
ditchVSfield_veg_siteArea_gaus <- glmmTMB(log_grass_forb ~ cover_type*plot_type + julian.date_scaled +
                              (1|area/site_id),
                            family = gaussian(), 
                            data = surveys)
```
...
```{r}
#simulate residuals
residuals_ditchVSfield_veg_siteArea_gaus <- simulateResiduals(fittedModel = ditchVSfield_veg_siteArea_gaus, plot = TRUE)

##model fit is good enough.  
```

Try accounting for different variance between cover types.

```{r}
ditchVSfield_veg_siteArea_hetVar <- glmmTMB(log_grass_forb ~ 
                                              cover_type*plot_type + 
                                              julian.date_scaled +
                                              (1|area/site_id),
                                            dispformula = ~ cover_type,
                                            family = gaussian(), 
                                            data = surveys)
```
...
```{r}
#simulate residuals
residuals_ditchVSfield_veg_siteArea_hetVar <- simulateResiduals(fittedModel = ditchVSfield_veg_siteArea_hetVar, plot = TRUE)

##model fit is worse than gaussian 
```

Stick with gaussian. Print results

```{r}
summary(ditchVSfield_veg_siteArea_gaus)
```
...
pairs(emmeans(ditchVSfield_veg_siteArea_gaus, ~ cover_type | plot_type))
```

Find effect of plot type within each cover type

```{r}
pairs(emmeans(ditchVSfield_veg_siteArea_gaus, ~ plot_type | cover_type))
```
...
pred_veg <- predict(ditchVSfield_veg_siteArea_gaus,
                            newdata = veg_data,
                            se.fit = TRUE,
                            re.form = NA,
                           type = "link")
...
```{r}
# Fit a simple Poisson
pois_mod <- glm(nest_searching_queens ~ 1, family = poisson, data = surveys)
