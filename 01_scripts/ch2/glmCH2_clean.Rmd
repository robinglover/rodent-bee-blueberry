...
#plot
ggplot(pred_floral_veg_df, aes(x = cover_type, y = mean.class, color = plot_type)) +
...
#Plot
ggplot(pred_rodent_floral_df, aes(x = bombus_floral_abun_scaled, y = fit, color = plot_type)) +
  geom_line(size = 1.5) +
...
#Plot
ggplot(pred_nsq_plottype_df, aes(x = plot_type, y = cond_fit, color = cover_type)) +
  geom_point(size = 8, position = position_dodge(0.6)) +
...
# Plot zi predictions
ggplot(pred_nsq_plottype_zi_df, aes(x = plot_type, y = zi_fit, color = cover_type)) +
  geom_point(size = 8,  position = position_dodge(0.6)) +
...
# What features (% veg, floral abun, burrow number) in the ditches improve NSQ and worker abundance? (Looking at ditch data only).
