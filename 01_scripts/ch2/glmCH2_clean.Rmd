...
nsq_glm_plotType_areaSite <- glmmTMB(nest_searching_queens ~ plot_typebee_season_type +
                              (1|area/site_id),
                            ziformula = ~plot_type +
                              bee_season_type +
...
# Get predicted conditional probabilities and standard errors
pred_worker_veg_cond <- predict(workers_veg_noArea_zipois,
...
pred_worker_veg_df <- cbind(worker_data_veg,
                 cond_fit = pred_worker_veg_cond$fit,
                 cond_se = pred_worker_veg_cond$se.fit)

ggplot(pred_worker_veg_df, aes(x = grass_forb_perc_scaled, y = cond_fit, color = plot_type)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = cond_fit - 1.96*cond_se, ymax = cond_fit + 1.96*cond_se), alpha = 0.2) +
  labs(title = "Conditional Model Predictions (Log-scale)",
       x = "Grass and Forb % (scaled)",
...
```{r}
# Get predicted conditional probabilities and standard errors
pred_worker_veg_zi <- predict(workers_veg_noArea_zipois,
                            newdata = worker_data_veg,
                            type = "zprob",
                            se.fit = TRUE,
                            re.form = NA)
...
# Combine predictions with data frame
pred_worker_veg_df <- cbind(worker_data_veg,
                 zi_fit = pred_worker_veg_zi$fit,
                 zi_se = pred_worker_veg_zi$se.fit)

ggplot(pred_worker_veg_df, aes(x = grass_forb_perc_scaled, y = zi_fit, color = plot_type)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = zi_fit - 1.96*zi_se, ymax = zi_fit + 1.96*zi_se), alpha = 0.2) +
  labs(title = "Zero-inflation Model Predictions (Presence/Absence)", y = "Predicted Probability of Structural Zero") +
  theme_classic()
...
# Get predicted conditional probabilities and standard errors
pred_worker_floral_cond <- predict(workers_veg_noArea_zipois,
...
pred_worker_floral_df <- cbind(worker_data_floral,
                 cond_fit = pred_worker_floral_cond$fit,
                 cond_se = pred_worker_floral_cond$se.fit)

ggplot(pred_worker_floral_df, aes(x = bombus_floral_abun_scaled, y = cond_fit, color = plot_type)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = cond_fit - 1.96*cond_se, ymax = cond_fit + 1.96*cond_se), alpha = 0.2) +
  labs(title = "Conditional Model Predictions (Log-scale)",
       x = "Floral abundance (scaled)",
...
```{r}
# Get predicted conditional probabilities and standard errors
pred_worker_floral_zi <- predict(workers_veg_noArea_zipois,
                            newdata = worker_data_floral,
                            type = "zprob",
                            se.fit = TRUE,
                            re.form = NA)
...
# Combine predictions with data frame
pred_worker_floral_df <- cbind(worker_data_floral,
                 zi_fit = pred_worker_floral_zi$fit,
                 zi_se = pred_worker_floral_zi$se.fit)

ggplot(pred_worker_floral_df, aes(x = bombus_floral_abun_scaled, y = zi_fit, color = plot_type)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = zi_fit - 1.96*zi_se, ymax = zi_fit + 1.96*zi_se), alpha = 0.2) +
  labs(title = "Zero-inflation Model Predictions (Presence/Absence)", y = "Predicted Probability of Structural Zero") +
  theme_classic()
