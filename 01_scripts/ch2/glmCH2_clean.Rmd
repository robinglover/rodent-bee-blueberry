...
pred_nsq_plottype_df <- cbind(nsq_data_plottype,
                 cond_fit = pred_nsq_plottype_cond$fit,
                 cond_se = pred_nsq_plottype_cond$se.fit)

ggplot(pred_nsq_plottype_df, aes(x = plot_type, y = cond_fit)) +
  geom_point(size = 8) +
  geom_errorbar(aes(ymin = cond_fit - 1.96*cond_se, ymax = cond_fit + 1.96*cond_se), width = 0.2, position = position_dodge(0.6), linewidth = 1.5) +
  labs(x = "Plot type",
...
pred_worker_plottype_df <- cbind(worker_data_plottype,
                 fit = pred_worker_plottype$fit,
                 se = pred_worker_plottype$se.fit)

ggplot(pred_worker_plottype_df, aes(x = plot_type, y = fit)) +
  geom_point(size = 8) +
  geom_errorbar(aes(ymin = fit - 1.96*se, ymax = fit + 1.96*se), width = 0.2, position = position_dodge(0.6), linewidth = 1.5) +
  labs(x = "Plot type",
...
#plot
ggplot(pred_floral_veg_df, aes(x = grass_forb_perc_scaled, y = response, color = plot_type)) +
  geom_line(size = 1) +
...
#plot
ggplot(pred_floral_date_df, aes(x = julian.date_scaled, y = response, color = plot_type)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = asymp.LCL, ymax = asymp.UCL, fill = plot_type), alpha = 0.2, color = NA) +
  labs(
    title = "Predicted Bombus Floral Abundance (scaled)",
