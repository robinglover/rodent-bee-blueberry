...
#Plot
ggplot(pred_veg_date_df, aes(x = date_formatted, y = fit, color = plot_type)) +
...
#plot
ggplot(pred_floral_veg_df, aes(color = cover_type, y = mean.class, x = plot_type)) +
  geom_point(size = 8, position = position_dodge(0.6)) +
...
Effect of cover type within each plot type

```{r}
pairs(emmeans(workers_veg_noArea, ~ cover_type | plot_type))
```
