...
```{r}
rodent_plotType_areaOnly <- glmmTMB(num_burrows ~ plot_type +
                              (1|area), 
                            family = poisson, 
                            data = surveys)
...
#compare models
anova(rodent_plotType_areaOnly, rodent_plotType_siteOnly, rodent_plotType_areaSite)
##site only is best
```

2)  Check diagnostics of model.

-   significant deviations from uniformity within-groups. Significant KS test. Try negative binomial.

```{r}
#simulate residuals
residuals_rodent_plotType_siteOnly <- simulateResiduals(fittedModel = rodent_plotType_siteOnly, plot = TRUE)
```
...
```{r}
rodent_plotType_siteOnly_negBin <- glmmTMB(num_burrows ~ plot_type +
                              (1|site_id), 
                            family = nbinom2(), 
                            data = surveys)

#simulate residuals
residuals_rodent_plotType_siteOnly_negBin <- simulateResiduals(fittedModel = rodent_plotType_siteOnly_negBin, plot = TRUE)
```

3)  Plot results

```{r}
summary(rodent_plotType_siteOnly_negBin)
```
...
# Predictions for conditional model (count)
pred_rodent_plot <- predict(rodent_plotType_siteOnly_negBin, 
...
pred_rodent_plot_df <- cbind(rodent_data_plot,
                 fit = pred_rodent_plot$fit,
                 se = pred_rodent_plot$se.fit)

# Plot conditional predictions
ggplot(pred_rodent_plot_df, aes(x = plot_type, y = fit)) +
  geom_point(size=3, position = position_dodge(0.6)) +
  geom_errorbar(aes(ymin = fit - 1.96*se, ymax = fit + 1.96*se), width = 0.2, position = position_dodge(0.6))+
  labs(title = "Model Predictions (Log-scale)", y = "Predicted rodent burrow count (log-scale)") +
  theme_classic()
```

### Findings

There are significantly more rodent burrow in ditch plots than in field plots.
