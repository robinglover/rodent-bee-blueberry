...
2.  Check for zero-inflation in the data by comparing expected proportion of zeros in the dataset to the actual proportion of zeros. If observed \> expected, ZI model is justified.

-   use ZI Poisson model to account for high proportion of zeros in NSQ counts (~70% of plots had zero bumblebees), which exceeded expectations under a standard Poisson distribution (~45%).

```{r}
# Observed proportion of zeros
obs_zeros <- mean(surveys$n_bees == 0)

# Expected proportion of zeros under Poisson
lambda <- mean(surveys$n_bees)
...
3.  Try different random effects structures. Use zero-inflated model to account for high abundance of zeros in the dataset.
...
caste_glm_justPlot <- glmmTMB(n_bees ~ caste*julian.date_scaled*plot_type +
                            cover_type +
                              (1|plot_id),
                            ziformula = ~ caste*julian.date_scaled*plot_type +
                            cover_type +
...
caste_glm_justArea <- glmmTMB(n_bees ~ caste*julian.date_scaled*plot_type +
                            cover_type +
                              (1|area),
                            ziformula = ~ caste*julian.date_scaled*plot_type +
                            cover_type +
...
caste_glm_areaPlot <- glmmTMB(n_bees ~ caste*julian.date_scaled*plot_type +
                            cover_type +
                              (1|area/plot_id),
                            ziformula = ~ caste*julian.date_scaled*plot_type +
                            cover_type +
...
#compare models
anova(caste_glm_noRE, caste_glm_justSite, caste_glm_noArea, caste_glm_justPlot, caste_glm_justArea, caste_glm_areaPlot, caste_glm_full)
##no area model is best
```

4.  Check diagnostics of model
