...
Run tweedie model w full random effect structure
...
testQuantiles(residuals_ditchVSfield_veg_tweedie) #ns
testDispersion(residuals_ditchVSfield_veg_tweedie) #ns
testOutliers(residuals_ditchVSfield_veg_tweedie) #ns
testZeroInflation(residuals_ditchVSfield_veg_tweedie) #ns
```
...
1)  Run poisson model (no plot random effect since only one observation per plot)

```{r}
rodent_veg_full <- glmmTMB(num_burrows ~ cover_type*plot_type +
                              (1|area/site_id),
                            family = poisson, 
                            data = surveys)
```
...
```{r}
residuals_rodent_veg_full <- simulateResiduals(fittedModel = rodent_veg_full, plot = TRUE)
...
```{r}
rodent_veg_full_nbin <- glmmTMB(num_burrows ~ cover_type*plot_type +
                              (1|area/site_id),
                            family = nbinom2, 
                            data = surveys)
...
#check diagnostics
#simulate residuals
residuals_rodent_veg_full_nbin <- simulateResiduals(fittedModel = rodent_veg_full_nbin, plot = TRUE)
##looks good
```
...
```{r}
summary(rodent_veg_full_nbin)
```

Find effect of cover type in each plot type

```{r}
pairs(emmeans(rodent_veg_full_nbin, ~ cover_type | plot_type))
```

Find effect of plot type in each cover type

```{r}
pairs(emmeans(rodent_veg_full_nbin, ~ plot_type | cover_type))
```
...
# Get predicted probabilities and standard errors
pred_rodent_plot <- predict(rodent_veg_full_nbin,
...
# Get predicted probabilities and standard errors
pred_rodent_veg <- predict(rodent_veg_full_nbin,
...
1.  Run poisson with full random effect structure.
...
```{r}
#simulate residuals
residuals_nsq_glm_veg_full <- simulateResiduals(fittedModel = nsq_glm_veg_full, plot = TRUE)
```

4.  Plot results

```{r}
summary(nsq_glm_veg_full)
```
...
eff_slopes <- emtrends(nsq_glm_veg_full, ~ plot_type, var = "bombus_floral_abun_scaled")
...
pred_nsq_plottype_cond <- predict(nsq_glm_veg_full,
...
# Get predicted zero probabilities and standard errors
pred_nsq_covertype_cond <- predict(nsq_glm_veg_full,
...
pred_nsq_floral_cond <- predict(nsq_glm_veg_full,
...
# Get predicted probabilities and standard errors
pred_nsq_floral <- predict(nsq_glm_veg_full,
                            newdata = nsq_data_floral,
                            se.fit = TRUE,
                            re.form = NA,
                            type = "link")
...
# Combine predictions with data frame
pred_nsq_floral_df <- cbind(nsq_data_floral,
                 fit_log = pred_nsq_floral$fit,
                 se_log = pred_nsq_floral$se.fit)
...
#Plot
ggplot(pred_nsq_floral_df, aes(x = bombus_floral_abun_scaled, y = fit, color = plot_type)) +
  geom_jitter(
    data = subset(surveys, cover_type == "herbaceous" & bee_season_type == "nestsearching"),
    aes(x = bombus_floral_abun_scaled, y = nest_searching_queens, color = plot_type),
    width = 0.05,
    height = 0,
    size = 3,
    alpha = 0.2,
...
# Get predicted zero probabilities and standard errors
pred_nsq_season_cond <- predict(nsq_glm_veg_full,
...
1)  Run poisson model with full RE
...
```{r}
residuals_workers_veg_full <- simulateResiduals(fittedModel = workers_veg_full, plot = TRUE)
```

3.  Try negative binomial

```{r}
workers_veg_full_nbin <- glmmTMB(workers ~ cover_type*plot_type +
                                bombus_floral_abun_scaled*plot_type +
                              bee_season_type+
                              (1|area/site_id/plot_id),
                            family = nbinom2(), 
...
residuals_workers_veg_full_nbin <- simulateResiduals(fittedModel = workers_veg_full_nbin, plot = TRUE)
...
# Get predicted probabilities and standard errors
pred_worker_plottype <- predict(workers_veg_full_nbin,
...
# Get predicted probabilities and standard errors
pred_worker_covertype <- predict(workers_veg_full_nbin,
...
# Get predicted probabilities and standard errors
pred_worker_floral <- predict(workers_veg_full_nbin,
...
# Get predicted probabilities and standard errors
pred_worker_season <- predict(workers_veg_full_nbin,
